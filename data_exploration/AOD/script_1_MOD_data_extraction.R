#namanpaul
#extraction for MOD files for AOD data (Terra satellite)
...
MOD_input_path <- paste0(current_wd, '/input/MOD/')
MOD_output_path <- paste0(current_wd, '/output/MOD_mult/')

#satellite MOD or MYD (Terra/Aqua)
satellite_name <- 'MOD'
year_val <- 2019
...
process_MOD <- function(MOD_input_path, MOD_output_path, satellite_name, year_val){
  ...
  for(i in seq_along(date_range)){
    ...
    MOD_files <- list.files(path = paste0(MOD_input_path,year_val,"/",julian_day),
                            pattern = "\\.hdf$")
    
    if(!purrr::is_empty(MOD_files)){
      
      foreach(k = seq_along(MOD_files)) %do% {
        
        #get the list of subdatasets
        sds <- gdalUtils::get_subdatasets(paste0(MOD_input_path,year_val,
                                                 "/",julian_day,"/",MOD_files[k]))
        ...
        #joining these dfs
        MOD_data <- df_cor_optical_depth_1 %>% 
        ...
        MOD_data_with_lonlat <- merge(MOD_data,
                                      lon_lat_time,
                                      by = c("x","y"))
        #assigning a randomstring
        assign(paste("random", k, sep="_"), MOD_data_with_lonlat)
        
      }
      #list of dataframes
      list_df <- lapply(ls(pattern="random+"), function(x) get(x))
      
      day_MOD_data <- dplyr::bind_rows(list_df)
      
      #a vector of dates
      dates_MOD <- ymd(unique(day_MOD_data$ACQ_DATE))
      
      counter = 0
      foreach(j = dates_MOD) %do% {
        #create a smaller df to operate on
        singledate <- day_MOD_data %>% 
        ...
        #setwd(MOD_output_path)
       
        #save the data
        save(singledate,
             file = paste0('MOD_',j,'_',counter,'.RData'))
      ...
      #save(day_MOD_data, file = paste0(MOD_output_path,'/','MOD_data_',satellite_name,'_',as.Date(as.character(date_range[i]), "%Y%j"), '.RData'))
...
for(year_val in 2019){
  process_MOD(MOD_input_path, MOD_output_path, satellite_name, year_val)
}
